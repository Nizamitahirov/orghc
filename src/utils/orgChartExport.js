...
// exportToPrint: browser print dialog with A3 landscape optimisation
export const exportToPrint = async ({ darkMode, selectedCompany, setExportLoading, summary }) => {
    try {
        setExportLoading?.('print');

        // Capture full chart as image, then open a clean print window
        ...
        const statsHtml = summary
            ? `<div class="stats">
                ${summary.totalEmployees  ? `<span><b>${summary.totalEmployees}</b> Employees</span>` : ''}
                ${summary.totalManagers   ? `<span><b>${summary.totalManagers}</b> Managers</span>` : ''}
                ${summary.totalVacancies  ? `<span><b>${summary.totalVacancies}</b> Vacant</span>` : ''}
                ${summary.totalDepartments? `<span><b>${summary.totalDepartments}</b> Departments</span>` : ''}
               </div>`
            : '';
...
<head>
  <meta charset="utf-8" />
  <title>Org Chart – ${companyLabel}</title>
  <style>
    @page { size: A3 landscape; margin: 10mm; }
    * { box-sizing: border-box; margin: 0; padding: 0; }
    body { font-family: Helvetica, Arial, sans-serif; background: white; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    .header { display: flex; justify-content: space-between; align-items: flex-end; padding-bottom: 6px; border-bottom: 2px solid #2346a8; margin-bottom: 6px; }
    .header-left h1 { font-size: 16pt; color: #2346a8; font-weight: 700; }
    .header-left p  { font-size: 9pt; color: #4f5772; margin-top: 2px; }
    .header-right   { font-size: 8pt; color: #7a829a; text-align: right; }
    .stats { display: flex; gap: 16px; padding: 4px 8px; background: #f1f5f9; border-radius: 4px; margin-bottom: 6px; font-size: 8pt; color: #4f5772; }
    .stats span { display: flex; gap: 4px; align-items: center; }
    .stats b { color: #1e293b; }
    img { width: 100%; height: auto; display: block; border: 1px solid #e2e8f0; border-radius: 4px; }
    .footer { margin-top: 6px; border-top: 1px solid #e2e8f0; padding-top: 4px; font-size: 7pt; color: #94a3b8; text-align: center; }
  </style>
</head>
<body>
  <div class="header">
    ...
    <div class="header-right">
      ${new Date().toLocaleDateString('az-AZ', { year: 'numeric', month: 'long', day: 'numeric' })}<br/>
      Almet HRIS
    </div>
  </div>
  ${statsHtml}
  <img src="${dataUrl}" />
  <div class="footer">Generated by Almet HRIS &nbsp;•&nbsp; ${new Date().toLocaleString('az-AZ')}</div>
  <script>window.onload = () => { window.print(); window.onafterprint = () => window.close(); }</script>
        ...
        console.error('Print export xətası:', err);
        alert(`Print uğursuz oldu: ${err.message}`);
    } finally {
