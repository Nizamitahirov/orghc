...
export default function JobCatalogPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('jobCatalog_activeView', activeView);
  }, [activeView]);

  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('jobCatalog_viewMode', viewMode);
  }, [viewMode]);

  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('jobCatalog_matrixView', matrixView);
  }, [matrixView]);

  useEffect(() => {
    if (typeof window !== 'undefined') localStorage.setItem('jobCatalog_showFilters', showFilters.toString());
  }, [showFilters]);

  // ==================== DATA LOADING ====================

  // FIX 1: useCallback ilə sabit referans - heç bir dependency yoxdur
  // loadEmployees ayrıca searchTerm/selectedFilters alır, state-dən oxumur
  ...
  }, []); // boş dependency - funksiya dəyişmir

  // FIX 2: loadInitialData useCallback + loadEmployees dependency
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      // employees-i parametrsiz yükləyirik (filter yoxdur hələ)
      await loadEmployees({});
  ...
  }, [loadEmployees]); // loadEmployees sabitdir, problem yoxdur

  // FIX 3: initial load - yalnız bir dəfə
  ...
  // FIX 4: search/filter dəyişdikdə employees yenidən yüklə
  // loadInitialData tamamlanana qədər çalışmasın
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => loadEmployees(params), 300);
    return () => clearTimeout(timer);
  }, [searchTerm, selectedFilters, loadEmployees]);
  ...
  // FIX 5: matrix view üçün hierarchy - hierarchyData dependency düzgün
  useEffect(() => {
    if (activeView === 'matrix' && !hierarchyData && !loading.hierarchy) {
      loadHierarchyData();
    }
  }, [activeView, hierarchyData, loading.hierarchy, loadHierarchyData]);

  // ==================== CRUD OPERATIONS ====================

  const openCrudModal = useCallback((type, mode = 'create', item = null) => {
    ...
    if (mode === 'edit' && item) {
      ...
      if (type === 'departments') {
        ...
        if (bfId) formDataInit.business_function = bfId;
      }
      ...
      if (type === 'units') {
        const deptId = item.department_id ||
          ...
          item.department;
        if (deptId) formDataInit.department = deptId;
      }
      ...
      if (type === 'departments') { cleanFormData.business_function_ids = []; cleanFormData.business_function = null; }
      if (type === 'units') { cleanFormData.department_ids = []; cleanFormData.department = null; }
  ...
  const handleCrudSubmit = useCallback(async (e) => {
    e.preventDefault();
    ...
    try {
      ...
      if (crudModalType === 'departments') {
        if (crudModalMode === 'create') {
          ...
          submitData.business_function_ids = Array.isArray(bfIds) ? bfIds : [bfIds];
        } else {
          submitData.business_function_ids = [formData.business_function];
        }
      }
      ...
      if (crudModalType === 'units') {
        if (crudModalMode === 'create') {
          ...
          submitData.department_ids = Array.isArray(deptIds) ? deptIds : [deptIds];
        } else {
          submitData.department_ids = [formData.department];
        }
      }
      ...
      const apiMap = {
        business_functions: { create: referenceDataAPI.createBusinessFunction, update: referenceDataAPI.updateBusinessFunction },
        departments: { create: referenceDataAPI.createDepartment, update: referenceDataAPI.updateDepartment },
        units: { create: referenceDataAPI.createUnit, update: referenceDataAPI.updateUnit },
        job_functions: { create: referenceDataAPI.createJobFunction, update: referenceDataAPI.updateJobFunction },
        job_titles: { create: referenceDataAPI.createJobTitle, update: referenceDataAPI.updateJobTitle },
        position_groups: { create: referenceDataAPI.createPositionGroup, update: referenceDataAPI.updatePositionGroup }
      };

      if (!apiMap[crudModalType]) throw new Error(`Unknown CRUD type: ${crudModalType}`);

      const itemId = selectedItem?.value || selectedItem?.id;
      if (crudModalMode === 'edit' && !itemId) throw new Error('Item ID is missing for update operation');

      if (crudModalMode === 'create') {
        await apiMap[crudModalType].create(submitData);
      } else {
        await apiMap[crudModalType].update(itemId, submitData);
      }

      await loadInitialData();
      showSuccess(`Successfully ${crudModalMode === 'create' ? 'created' : 'updated'} ${crudModalType.replace(/_/g, ' ')}`);
      closeCrudModal();
      ...
      if (error?.response?.data) {
        ...
        else if (d.non_field_errors) errorMsg = [].concat(d.non_field_errors).join(', ');
        else {
          const fieldErrors = Object.entries(d)
            .filter(([k]) => !['message', 'detail', 'status'].includes(k))
            .map(([f, e]) => `${f}: ${[].concat(e).join(', ')}`).join('; ');
  ...
  }, [formData, crudModalType, crudModalMode, selectedItem, loadInitialData, closeCrudModal, showSuccess, showError]);
  ...
  const handleDelete = useCallback(async (type, item) => {
    ...
    try {
      ...
      if (!deleteMap[type]) throw new Error(`Unknown delete type: ${type}`);
  ...
  const stats = useMemo(() => {
    ...
    return { totalJobs: jobCatalogData.length, totalEmployees: statistics.total_employees || 0 };
  ...
  const matrixData = useMemo(() => {
    ...
    positionGroupsState.forEach(pg => {
      ...
      columns.forEach(col => {
        matrix[hierarchyName][col] = jobCatalogData.filter(job => {
  ...
  const clearFilters = useCallback(() => {
    setSelectedFilters({ business_function: '', department: '', unit: '', job_function: '', position_group: '' });
    setSearchTerm('');
  }, []);
  ...
  const contextValue = useMemo(() => ({
    ...
    activeView, viewMode, matrixView, showFilters,
    selectedJob, showCrudModal, crudModalType, crudModalMode, selectedItem,
    employees, statistics, businessFunctions, departments, units,
    jobFunctions, jobTitles, positionGroupsState, hierarchyData,
    searchTerm, selectedFilters, loading, errors, formData,
  ...
  if (loading.initial) return <LoadingSpinner message="Loading Job Catalog..." />;
  if (errors.initial) return <ErrorDisplay error={errors.initial} onRetry={loadInitialData} />;

  return (
    <DashboardLayout>
      <div className="p-3 sm:p-4 bg-almet-mystic dark:bg-gray-900 min-h-screen">
        <div className="mb-4">
          <h1 className="text-lg font-semibold text-gray-900 dark:text-white mb-1">Job Catalog</h1>
        ...
        {activeView === 'hierarchical' && <HierarchicalTableView context={{ ...contextValue, employees }} />}
