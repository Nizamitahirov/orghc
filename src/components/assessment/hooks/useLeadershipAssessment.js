...
const useLeadershipAssessment = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const [
        ...
        assessmentApi.employeeLeadership.getAll(employeeParams),
  ...
  const filteredPositionAssessments = useMemo(() => positionAssessments.filter(assessment =>
    assessment.position_group_name?.toLowerCase().includes(searchTerm.toLowerCase())
  ), [positionAssessments, searchTerm]);

  const filteredEmployeeAssessments = useMemo(() => employeeAssessments.filter(assessment =>
    (assessment.employee_name?.toLowerCase().includes(searchTerm.toLowerCase())) &&
    (selectedStatus === '' || assessment.status === selectedStatus)
  ), [employeeAssessments, searchTerm, selectedStatus]);
  ...
  return {
    ...
    expandedChildGroups, setExpandedChildGroups,
    ...
    editGradeLevels, setEditGradeLevels,
...
export default useLeadershipAssessment;
