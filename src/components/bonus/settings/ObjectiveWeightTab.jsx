...
export default function ObjectiveWeightTab({ bonusYear, dark }) {
  ...
  /* ── Load: with_position_groups endpoint-dən gəlir ── */
  const load = async () => {
    setLoading(true);
    try {
      const { data } = await objectiveWeightConfigService.withPositionGroups(bonusYear.id);
      const list = Array.isArray(data) ? data : [];
      setRows(list);
      const init = {};
      list.forEach((r) => { init[r.position_group_id] = r.weight ?? ""; });
      setEdited(init);
    } catch {
      showError("Failed to load objective weights.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveRow = async (row) => {
    ...
    const payload = {
      ...
      position_group: row.position_group_id,
    ...
    <div>
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div>
          ...
          <p className={`text-xs mt-1 ${sub}`}>
            Set the adjusted weight for each position group used in objective bonus calculation.
          </p>
        </div>
      </div>
      ...
      <div className={`flex items-start gap-3 px-4 py-3.5 rounded-xl border mb-6
        ...
        <p className={`text-xs leading-relaxed ${sub}`}>
          <b>Example:</b> Employee has objectives totalling <b>100%</b>. Position adjusted weight = <b>75%</b>.
          Each objective's effective weight is scaled so total = <b>75%</b>.
          Bonus = Salary × scaled_weight% × Rating%.
        </p>
      </div>
      ...
      <div className={`rounded-2xl border overflow-hidden mb-4
        ...
        {/* Table header */}
        <div
          className={`grid items-center px-5 py-3 text-[10px] font-bold uppercase tracking-widest border-b
