...
import {
  ...
  XCircle,  CheckSquare, AlertCircle,  ArrowRight
} from 'lucide-react';
...
export default function EmployeeDetailPerformance({
  ...
  const downloadExcel = async (recordId) => {
    try {
      const name = `performance-${employeeData?.name || employeeData?.employee_name || 'employee'}-${new Date().getFullYear()}.xlsx`;
  ...
  // ── Approval helpers ──
  const openApproval = (record, type, label, description) => {
    setSelectedRecord(record);
    setCurrentAction({ type, label, description });
  ...
  const executeApproval = async () => {
    ...
    try {
      ...
      else if (currentAction.type === 'approve_final')
        ...
        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-amber-900/20 border-amber-700/50' : 'bg-amber-50 border-amber-200'}`}>
          ...
          <div className="space-y-2">
            {allActions.map((action, i) => (
              <div
                key={i}
                className={`flex items-center justify-between p-3 rounded-lg ${darkMode ? 'bg-amber-900/30 border-amber-700/30' : 'bg-white border-amber-200'} border`}
              >
    ...
    <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-lg border shadow-sm overflow-hidden`}>
      <div className={`px-4 py-3 flex items-center justify-between ${darkMode ? 'hover:bg-gray-750' : 'hover:bg-gray-50'} transition-colors`}>
        <button onClick={onToggle} className="flex items-center gap-2.5 flex-1">
          ...
          {badge > 0 && (
            <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${darkMode ? 'bg-almet-sapphire/20 text-almet-astral' : 'bg-almet-mystic text-almet-sapphire'}`}>
              {badge}
            </span>
          )}
        </button>
...
// ─── RECORD CARD ─────────────────────────────────────────────────────────────
function RecordCard({ record, darkMode, getPeriodLabel, getPeriodColor, getScoreColor, formatDate, onView, onDownload, employeeActions }) {
  ...
  const STATUS_LABELS = {
    DRAFT: 'Draft',
    PENDING_EMPLOYEE_APPROVAL: 'Pending Your Approval',
  ...
  const getStatusColor = (status) => {
    const map = {
      DRAFT: darkMode ? 'text-gray-400' : 'text-gray-500',
      PENDING_EMPLOYEE_APPROVAL: darkMode ? 'text-amber-400' : 'text-amber-700',
      PENDING_MANAGER_APPROVAL: darkMode ? 'text-blue-400' : 'text-blue-700',
      NEED_CLARIFICATION: darkMode ? 'text-red-400' : 'text-red-700',
      APPROVED: darkMode ? 'text-green-400' : 'text-green-700',
      COMPLETED: darkMode ? 'text-emerald-400' : 'text-emerald-700',
    };
    return map[status] || '';
  };
  ...
  return (
    <div className={`${darkMode ? 'bg-gray-750 border-gray-700' : 'bg-gray-50 border-gray-200'} rounded-lg border p-4 hover:shadow-sm transition-shadow`}>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-0.5">
              ...
              {record.approval_status && (
                <span className={`text-xs font-medium ${getStatusColor(record.approval_status)}`}>
                  • {STATUS_LABELS[record.approval_status] || record.approval_status}
                </span>
              )}
            </div>
          </div>
      ...
      {/* Score pills - FIX: use flat fields */}
      {hasScores && (
        <div className="flex flex-wrap gap-2 mb-3">
          {objPct > 0 && (
            <ScorePill label="Objectives" value={`${objPct.toFixed(1)}%`} color={getScoreColor(objPct)} darkMode={darkMode} />
          )}
          {compPct > 0 && (
            <ScorePill label="Competencies" value={`${compPct.toFixed(1)}%`} color={getScoreColor(compPct)} darkMode={darkMode} />
          )}
          {overallPct > 0 && (
            <ScorePill label="Overall" value={`${overallPct.toFixed(1)}%`} color={getScoreColor(overallPct)} darkMode={darkMode} />
          )}
      ...
      { employeeActions.length > 0 && (
        <div className={`flex flex-wrap gap-2 pt-2 border-t border-dashed ${darkMode ? 'border-amber-700/40' : 'border-amber-300/50'}`}>
  ...
  return (
    <div className="text-center py-8">
      ...
      <p className={`text-xs ${darkMode ? 'text-gray-500' : 'text-gray-600'}`}>
        Performance records will appear here once initialized.
      </p>
    </div>
  );
}
      ...
      <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl w-full max-w-md border shadow-xl`}>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className={`p-3 rounded-lg ${darkMode ? 'bg-blue-900/20 border border-blue-800/30' : 'bg-blue-50 border border-blue-200'}`}>
            <p className={`text-xs ${darkMode ? 'text-blue-400' : 'text-blue-700'}`}>
              ✓ By approving, you confirm that you have reviewed and accept this performance assessment.
            </p>
          </div>
        </div>
        ...
        <div className={`px-5 py-4 ${darkMode ? 'bg-gray-750 border-gray-700' : 'bg-gray-50 border-gray-200'} border-t flex gap-2 justify-end`}>
          ...
          <button onClick={onConfirm} disabled={loading} className="px-4 py-2 rounded-lg text-sm font-semibold bg-green-600 hover:bg-green-700 text-white transition-all disabled:opacity-50 flex items-center gap-1.5">
            {loading ? <><RefreshCw size={14} className="animate-spin" /> Processing...</> : <><CheckCircle size={14} /> Confirm</>}
          </button>
        </div>
      </div>
    </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className={`${darkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl w-full max-w-4xl border shadow-xl my-8`}>
        <div className={`px-5 py-4 border-b ${darkMode ? 'border-gray-700 bg-gray-800' : 'border-gray-200 bg-white'} flex items-center justify-between sticky top-0 z-10 rounded-t-xl`}>
          ...
          <div className="flex items-center gap-2">
            <button onClick={onDownload} className={`p-2 rounded-lg transition-colors ${darkMode ? 'text-green-400 hover:bg-green-500/20' : 'text-green-600 hover:bg-green-50'}`}>
              <Download size={18} />
            </button>
        ...
        <div className={`px-5 py-4 ${darkMode ? 'bg-gray-750 border-gray-700' : 'bg-gray-50 border-gray-200'} border-t rounded-b-xl flex justify-end`}>
          <button onClick={onClose} className="px-5 py-2 rounded-lg text-sm font-semibold bg-almet-sapphire hover:bg-almet-astral text-white transition-all">
            Close
          </button>
        </div>
      </div>
...
function SectionTitle({ darkMode, icon, title, color }) {
  ...
  return (
    <h4 className={`text-sm font-semibold flex items-center gap-2 ${colors[color]}`}>
      {icon} {title}
    </h4>
  );
}
