...
const handleSoftDelete = async () => {
  if (selectedCount === 0) {
    showWarning('Please select employees to end employment for');
    return;
  }
  ...
  try {
    if (isSoft) {
      const result = await archiveEmployeesService.bulkSoftDeleteEmployees(
        selectedEmployees, notes, terminationDate, exitType
      );
      await refreshAll();
      onClose();
      showSuccess(result.message || `Successfully ended employment for ${selectedCount} employee${selectedCount !== 1 ? 's' : ''}!`);
      if (result.data?.vacant_positions_created > 0) {
        setTimeout(() => showInfo(`Created ${result.data.vacant_positions_created} vacant position(s) automatically.`), 1000);
      }
    } else {
      const result = await archiveEmployeesService.bulkHardDeleteEmployees(
        selectedEmployees, notes || 'Permanent deletion', true, terminationDate, exitType
      );
      await refreshAll();
      onClose();
      ...
      if (result.data?.archives_created > 0) {
        setTimeout(() => showInfo(`Created ${result.data.archives_created} archive record(s) for audit purposes.`), 1000);
      }
    }
