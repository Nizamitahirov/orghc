...
import { Eye, ListTodo, FolderKanban, Plus } from 'lucide-react';
...
export default function ListView({ tasks, onTaskClick, onCreateTask, darkMode }) {
  ...
  return (
    <div className={`${bgCard} border ${brd} rounded-xl shadow-sm overflow-hidden`}>
      ...
      {tasks.length === 0 && (
        <div className="py-20 text-center px-6">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 ${darkMode ? 'bg-almet-sapphire/10' : 'bg-almet-sapphire/8'}`}>
            <ListTodo size={28} className="text-almet-sapphire opacity-70" />
          </div>
          <p className={`text-base font-semibold ${txt} mb-1.5`}>Tapşırıq yoxdur</p>
          <p className={`text-sm ${txtMut} mb-5`}>Yeni tapşırıq yaradaraq başlayın</p>
          {onCreateTask && (
            <button
              onClick={onCreateTask}
              className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-lg bg-almet-sapphire text-white hover:bg-almet-cloud-burst transition-colors"
            >
              <Plus size={15} />
              Tapşırıq yarat
            </button>
