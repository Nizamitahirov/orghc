...
const NotificationDropdown = () => {
  ...
  const fetchEmails = async () => {
    try {
      setLoading(true);
      const data = await NotificationService.getOutlookEmails({ module: 'all', email_type: 'received', top: 10 });
      if (data.success) {
  ...
  const handleMarkAsRead = async (messageId, e) => {
    ...
    try {
      ...
      showSuccess(t('notifications.markedAsRead'));
    } catch (error) {
  ...
  const handleMarkAsUnread = async (messageId, e) => {
    ...
    try {
      ...
      setEmails(prev => prev.map(e => e.id === messageId ? { ...e, is_read: false } : e));
      setUnreadCount(prev => prev + 1);
      showSuccess(t('notifications.markedAsUnread'));
    } catch (error) {
  ...
  const handleMarkAllAsRead = async (e) => {
    ...
    try {
      ...
      setUnreadCount(0);
      showSuccess(`${unreadEmails.length} ${t('notifications.markedAsRead')}`);
    } catch (error) {
  ...
  const getTimeAgo = (dateString) => {
    ...
    if (diffInMins < 1) return t('notifications.now');
          ...
          <div className="px-3 py-2.5 bg-gradient-to-r from-almet-sapphire to-almet-astral border-b border-almet-sapphire/20">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h3 className="text-xs font-bold text-white">{t('notifications.inbox')}</h3>
                  {unreadCount > 0 && (
                    <p className="text-[10px] text-white/80">{unreadCount} {t('notifications.unread')}</p>
                  )}
                </div>
              </div>
              {unreadCount > 0 && (
          ...
          <div className="max-h-[420px] overflow-y-auto">
            {loading ? (
              <div className="p-8 text-center">
                ...
                <p className="text-[11px] text-gray-500 dark:text-almet-bali-hai">{t('notifications.loading')}</p>
              </div>
            ) : emails.length === 0 ? (
              <div className="p-8 text-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-2 mb-1">
                          <p className={`text-[11px] truncate flex-1 ${
                            !email.is_read ? 'font-semibold text-gray-900 dark:text-white' : 'text-gray-600 dark:text-gray-400'
                          }`}>
                        ...
                        <p className={`text-[11px] line-clamp-2 leading-relaxed ${
                          !email.is_read ? 'text-gray-700 dark:text-gray-300' : 'text-gray-500 dark:text-gray-400'
                        }`}>
                          {email.subject}
                        </p>
                      </div>
