// ═══════════════════════════════════════════════════════
// src/components/si/settings/SICompanyKPITab.jsx
// Yalnız KPI adları + çəkilər — scale ayrıca tabdadır
// ═══════════════════════════════════════════════════════
"use client";
import { useState, useEffect } from "react";
import { siCompanyKPIService } from "@/services/siService";
import { useToast } from "@/components/common/Toast";
import { Plus, Pencil, Trash2, Target } from "lucide-react";

export default function SICompanyKPITab({ dark, config }) {
  ...
  const load = () =>
    siCompanyKPIService.list(config.id)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (form.id) await siCompanyKPIService.update(form.id, payload);
      else         await siCompanyKPIService.create(payload);
      setForm(null);
      await load();
  ...
  const handleDelete = async id => {
    setDel(id);
    try { await siCompanyKPIService.delete(id); await load(); showSuccess("KPI deleted."); }
    ...
    <div>
      ...
      <div className="flex items-start justify-between mb-8">
        <div className="flex items-start gap-4">
          ...
          <div>
            <h2 className={`text-base font-bold ${text}`}>Company KPIs</h2>
            <p className={`text-xs mt-1 ${sub}`}>
              Define company-level KPIs and their weights.
              <span className={`ml-2 font-bold ${weightColor}`}>
      ...
      {kpis.length === 0 ? (
        <div className={`text-center py-14 rounded-2xl border-2 border-dashed
          ...
          <Target size={26} className="mx-auto mb-3 opacity-30" />
          <p className="text-sm font-medium">No KPIs yet</p>
          <p className="text-xs mt-1 opacity-70">Click "Add KPI" to get started</p>
        </div>
      ) : (
        <div className={`rounded-2xl border overflow-hidden ${dark ? "border-[#1e1e1e]" : "border-gray-200"}`}>
          ...
          <div className={`grid items-center px-5 py-2.5 border-t
            ...
            <span />
            <span className={`text-sm font-black text-right tabular-nums ${weightColor}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className={`w-full max-w-sm rounded-2xl border shadow-2xl overflow-hidden
            ...
            <div className={`px-6 py-4 border-b ${dark ? "border-white/[0.06]" : "border-gray-100"}`}>
              <h3 className={`text-sm font-bold ${text}`}>{form.id ? "Edit KPI" : "Add Company KPI"}</h3>
            </div>
            <div className="px-6 py-5 space-y-4">
              {[
                { key: "kpi_name",     label: "KPI Name *",            type: "text",   ph: "e.g. Sales Volume (ton)" },
                { key: "kpi_code",     label: "Code *",                type: "text",   ph: "e.g. SALES_VOLUME"       },
                { key: "weight",       label: "Weight (%)",            type: "number", ph: "e.g. 30"                 },
                { key: "display_order",label: "Display Order",         type: "number", ph: "0"                       },
